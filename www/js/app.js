     ...
     $('#download').click( function()
		{
		    try {
				...
				if (sessionStorage.platform.toLowerCase() == "android") {
					window.resolveLocalFileSystemURL(cordova.file.applicationStorageDirectory,onFileSystemSuccess, onError);
				}
				else {
		...
		function onFileSystemSuccess(fileSystem) {
			...
			entry.getDirectory("www", {create: true, exclusive: false}, onGetDirectorySuccess, onGetDirectoryFail);
		};
		function onGetDirectorySuccess(dir) {
			dir.getDirectory("content", {create: true, exclusive: false}, onGetDirectorySuccess1, onGetDirectoryFail);
		};
		...
		function gotFileEntry(fileEntry) {
				var uri = encodeURI("https://datasyn.000webhostapp.com/tree.json");
		...
		function onFileSystemSuccessDelete(fileSystem) {
			...
			entry.getDirectory("www/content", {create: true, exclusive: false}, onGetDirectorySuccessDelete, onGetDirectoryFail);
		...
		function gotFileEntryDelete(fileEntry) {
			fileEntry.remove();
			var uri = encodeURI("http://SERVER_IP:PORT/tree.json");
				fileTransfer.download(uri,cdr.nativeURL+filename,
					function(entry) {
